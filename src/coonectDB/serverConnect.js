...
//     db.query("INSERT INTO departmentdetails (department) VALUES (?)", [department], (err, result) => {
//         if (err) return res.status(500).json({ message: "Database error", error: err });
//         res.json({ success: true, message: "Department added successfully", departmentID: result.insertId });
//     });
// });
...
//     db.query("UPDATE departmentdetails SET department = ? WHERE departmentID = ?", [department, id], (err) => {
//         if (err) return res.status(500).json({ message: "Database error", error: err });
//         res.json({ success: true, message: "Department updated successfully" });
//     });
// });
...
app.put("/api/signindetails/:id", verifyToken, (req, res) => {
    ...
    db.query(
        "UPDATE signindetails SET roleName = ?, department = ? WHERE id = ?",
